...
def fused_silu_ffn(x: torch.Tensor, layer_name: str) -> torch.Tensor:
    ...
    (registered in static_forward_context during model init) and executes
    gate_up_proj -> act_fn -> down_proj, reusing whatever quantization /
    tensor-parallel logic the underlying modules provide.
    """
    forward_context = get_forward_context()
    mlp = forward_context.no_compile_layers[layer_name]
    intermediate, _ = mlp.gate_up_proj(x)
    intermediate = mlp.act_fn(intermediate)
    output, _ = mlp.down_proj(intermediate)
    return output
